...
sequence of actions (a 'plan') that lead to completing the required task.

Do not express control flow with "if", indentation or the like. Plans are simple flat lists of function names,
optionally followed by a colon and a textual explanation of the step.
...
def plan(query):
    ...
    response = run_completion(planner_prompt)
    ...
    for rl in rlines:
        ...
        if ":" in rest:
            ...
            notes = None
        _validate_fct(fct)
        final_plan.append({"function": fct, "notes": notes})
    return final_plan
